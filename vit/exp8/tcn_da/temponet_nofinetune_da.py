...
from src.models.TEMPONet import TEMPONet
...
class DB6SessionAugmented(Dataset):
    ...
    def windowing(self, steady=True, n_classes='7+1', image_like_shape=False, steady_margin_s=1.5, **kwargs):
        ...
        X_windows, R_windows, Y_windows = windowing(self.X, self.R, self.Y, steady=steady, steady_margin_s=steady_margin_s, **kwargs)
        self.no_augmented_len = Y_windows.shape[0]

        self.time_stretches =  [.75, 1.25, 1.5]
        # Time stretch d.a.
        for f in self.time_stretches:
            X_windows_, R_windows_, Y_windows_ = resample_and_windowing(self.X, self.R, self.Y, factor=f, steady_margin_s=steady_margin_s * f)
            X_windows = np.concatenate((X_windows, X_windows_), axis=0)
        ...
        self.amplitude_stretches = [1, -1, .8, 1.2]
    ...
    def __getitem__(self, idx):
        x = self.X[idx % self.base_len] * self.amplitude_stretches[idx // self.base_len]
...
def main_1test(chunk_idx):
    ...
    if ds_cache['ds_config'] != ds_config:
        ...
        ds = load(open("/home/fbmorghet/vit/exp8/session_augment_fix2/train_ds.pickle", "rb"))
...
def main_train_subject(chunk_idx):
    ...
    ds = load(open(f"/home/fbmorghet/vit/exp8/session_augment_fix2/flat/train_ds_subject{subject}_minmaxTrue.pickle", "rb"))

    test_datasets_steady = [DB6MultiSession(folder=os.path.expanduser('~/DB6'), subjects=[subject], sessions=[i], 
                                            steady=True, n_classes=n_classes, minmax=(ds.X_min, ds.X_max), image_like_shape=False) \
                            for i in test_sessions]
    test_datasets = [DB6MultiSession(folder=os.path.expanduser('~/DB6'), subjects=[subject], sessions=[i], 
                                     steady=False, n_classes=n_classes, minmax=(ds.X_min, ds.X_max), image_like_shape=False) \
                            for i in test_sessions]
    ...
    "training_config": [
        {
         ...
         "batch_size": 64,
         "optim": "SGD",
         "optim_hparams": {"lr": 1e-3, "momentum": 0.9, "dampening": 0, "weight_decay": 0},
         "lr_scheduler": "MultiStepLR",
         "lr_scheduler_hparams": {"milestones": [9, 19], "gamma": .1}
        }
    ]
}
