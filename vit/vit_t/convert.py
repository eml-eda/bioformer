...
def convert_state_dict(state_dict, inplace=True):
    ...
    # Fix path embedding
    state_dict['patch_conv.weight'] = state_dict.pop('to_patch_embedding.1.weight').reshape(64, 30, 14).permute(0, 2, 1)
...
state_dict = torch.load(f"vit_h8patch30.pth", map_location=torch.device('cpu'))
state_dict = convert_state_dict(state_dict)
torch.save(state_dict, f"vit_h8patch30_conv_emb.pth")