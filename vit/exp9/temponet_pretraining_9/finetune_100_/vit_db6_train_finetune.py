...
def train(net, net_name, ds, k, bootstrap, training_config, test_ds=None):
    ...
    while epoch < total_epochs:
        ...
        if (val_preds[0] / val_preds.sum()) < .90:
            ok += 1
        else:
            ok = 0  # reset, potential infinite loop in case training is very instable

        if total_epochs == float('inf') and ok >= 20 and abs(losses_accs[epoch]['val_acc'] - losses_accs[epoch - 10]['val_acc']) < (1 / 100):
            scheduler.step()
            total_epochs = epoch + 20
...
def main_train_subject(chunk_idx):
    ...
    ds = DB6MultiSession(folder=os.path.expanduser('~/DB6'), subjects=[subject], sessions=train_sessions, steady=steady, n_classes=n_classes, minmax=minmax, image_like_shape=False).to('cuda')
    ...
    results_ = []
    for i, config in enumerate(configs, start=1):
        config['pretrained'] = f"../vit_dim64_h8_{subject - 1}_1_epoch20.pth"
        results = {}
    ...
    "training_config": [
        {
         ...
         "lr_scheduler": "StepLR",
         "lr_scheduler_hparams": {"step_size": 1, "gamma": .1}
        },
    ]
}
