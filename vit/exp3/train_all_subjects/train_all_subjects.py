...
def train_subject(subject, train_sessions=(0,), test_sessions=(1,2,3,4,5,6,7,8,9), steady=True, n_classes='7+1', bootstrap='no', folder='~/DB6'):
    ...
    ds = DB6MultiSession(folder=os.path.expanduser(folder), subject=subject, sessions=train_sessions, steady=steady, n_classes=n_classes, minmax=True, image_like_shape=True).to('cuda')

    test_datasets_steady = [DB6MultiSession(folder=os.path.expanduser(folder), subject=subject, sessions=[i], 
                                            steady=True, n_classes=n_classes, minmax=(ds.X_min, ds.X_max), image_like_shape=True).to('cuda') \
                            for i in test_sessions]
    test_datasets = [DB6MultiSession(folder=os.path.expanduser(folder), subject=subject, sessions=[i], 
