...
def load_dataset(dataset_dir, subject):
    ...
    minmax_picklename = f'./ds_minmax.pickle'
...
def load_model(heads, patch_size):
    net = ViT(patch_length=patch_size, heads=heads)
    net.to(device)
    net.eval()
    net.load_state_dict((torch.load(f"vit_h{heads}patch{patch_size}.pth")))
    return net
...
patch_size, heads = 30, 2
depth = 2 if heads == 2 else 1

net = ViT(patch_length=patch_size, heads=heads, depth=depth)
net.to(device)
net.eval()
net.load_state_dict((torch.load(f"vit_h{heads}patch{patch_size}_linear_emb.pth", map_location=torch.device('cpu'))))
