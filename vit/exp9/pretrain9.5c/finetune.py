...
def main_train_subject(chunk_idx):
    ...
    for i, config in enumerate(configs, start=1):
        ...
        for k in [0, 1]:
            ...
            config['pretrained'] = f"vit_dim64_h8_f{k}_{9 - (subject - 1)}_1_epoch100.pth"
            if config['pretrained'] is not None:
                net.load_state_dict(torch.load(config['pretrained']))
                print("Loaded checkpoint", config['pretrained'])

            losses_accs = train(net=net, net_name=f"{name_prefix}_{chunk_idx}_{i}_fold{k}", ds=ds, k=k, bootstrap=bootstrap, training_config=config['training_config'], test_ds=test_ds_5)
