...
class DB6Session(Dataset):
    ...
    def __init__(self, filename):
        ...
        self.data_augment_indices = np.array([-1])
    ...
    def __getitem__(self, idx):
        if not np.isin(idx, self.data_augment_indices):
            return self.X[idx], self.Y[idx]

        x = self.X[idx]

        # Random flip around zero
        x *= 2 * int(np.random.rand() > .5) - 1

        # Random time resample: slow down the signal for a random factor between 1 and 2
        # and take the central part of the slowed-down signal
        n = np.random.randint(150, 300) * 2 # Random even int between 300 and 598
        offset = (n - 300) // 2
        x = torch.tensor(signal.resample(x.cpu(), n, axis=2)[:, :, offset:n - offset], dtype=torch.float32).to(x.device)

        # amplitude jitter
        #x = x + torch.normal(mean=0, std=.1, size=(14, 1, 300))
        #x = x + torch.normal(mean=0, std=.1, size=(14, 1, 300)).to(x.device)
        #x = x + torch.normal(mean=0, std=.025, size=(14, 1, 300)).to(x.device)
        #x = x + torch.normal(mean=0, std=.05, size=(14, 1, 300)).to(x.device)
        #x = x + torch.normal(mean=0, std=.025, size=(14, 1, 300)).to(x.device)
        #x = x + torch.normal(mean=0, std=.05, size=(14, 1, 300)).to(x.device)
        x = x + torch.normal(mean=0, std=.025, size=(14, 1, 300)).to(x.device)
        # amplitude warp
        #x = x * torch.normal(mean=1, std=.2, size=(14, 1, 1)).to(x.device)
        x = x * torch.normal(mean=1, std=.2, size=(14, 1, 1)).to(x.device)
