...
def train(net, net_name, ds, k, bootstrap, training_config, test_ds=None, ds_add_sub=None):
...
def main_1test(chunk_idx):
    ...
    ds_config = dict(folder=os.path.expanduser('~/DB6'), subjects=subjects, sessions=list(range(n_sessions)), minmax=minmax, n_classes='7+1', steady=True, image_like_shape=False)
    if ds_cache['ds_config'] != ds_config:
        ds = DB6MultiSession(folder=os.path.expanduser('~/DB6'), subjects=subjects, sessions=list(range(n_sessions)), minmax=minmax, n_classes='7+1', steady=True, image_like_shape=False).to('cuda')
        add_sub = [a for a in range(1,11) if a not in subjects]
        assert len(add_sub) == 1
        ds_add_sub = DB6MultiSession(folder=os.path.expanduser('~/DB6'), subjects=add_sub, sessions=list(range(n_sessions)), minmax=(ds.X_min, ds.X_max), n_classes='7+1', steady=True, image_like_shape=False).to('cuda')
        test_ds = DB6MultiSession(folder=os.path.expanduser('~/DB6'), subjects=subjects, sessions=[n_sessions], minmax=(ds.X_min, ds.X_max), n_classes='7+1', steady=True, image_like_shape=False).to('cuda')
        if is_notebook():
            ds_cache['ds'] = ds
    ...
    for i, config in enumerate(configs, start=1):
        ...
        losses_accs = train(net=net, net_name=f"{name_prefix}_{chunk_idx}_{i}", ds=ds, k=1, bootstrap='no', training_config=config['training_config'], test_ds=test_ds, ds_add_sub=ds_add_sub)
    ...
    "training_config": [
        {
         ...
         "batch_size": 64,
         "optim": "SGD",
         "optim_hparams": {"lr": 10**-3, "momentum": 0.9, "dampening": 0, "weight_decay": 0},
         "lr_scheduler": "StepLR",
