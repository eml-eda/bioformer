# -*- coding: utf-8 -*-
"""vit_db6_pretrain.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/177Z86kRGu39CQbURrXSwystyUOA7rCYe
"""
...
from src.datasets.db6 import DB6Session
...
class ViT(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, pool = 'cls', channels = 3, dim_head = 64, dropout = 0., emb_dropout = 0., use_cls_token=True, sessions="ignore", subjects="ignore", training_config="ignore"):
        super().__init__()
...
def train(net, ds, k, bootstrap, test_ds, training_config):
    ...
    #exec(training_config, globals(), locals())
    print(epochs)
    print(optimizer)
    print(scheduler)
    ...
    for epoch in range(epochs):
        ...
        if ((epoch + 1) % 10) == 0:
            torch.save(net.state_dict(), f"vit_{i}_5sess_epoch{epoch+1}.pth")
...
configs_ = {
    ...
    "training_config": [
        ...
        #},
        {
         ...
         "optim_hparams": {"lr": 0, "betas": (.9, .999), "weight_decay": 0},
         "lr_scheduler": "CyclicLR",
         "lr_scheduler_hparams": {"base_lr": 1e-7, "max_lr": 1e-5, "step_size_up": 200, "step_size_down": None, "mode": 'triangular', "cycle_momentum": False},
        },
        {
         "epochs": 200,
         "optim": "Adam",
         "optim_hparams": {"lr": 0, "betas": (.9, .999), "weight_decay": 0},
         "lr_scheduler": "CyclicLR",
         "lr_scheduler_hparams": {"base_lr": 1e-7, "max_lr": 1e-4, "step_size_up": 200, "step_size_down": None, "mode": 'triangular', "cycle_momentum": False},
        },
    ]
}
...
for i, config in enumerate(configs, start=1):
    ...
    if n_sessions != prev_n_sessions:
        ...
        ds = DB6MultiSession(folder=os.path.expanduser('~/DB6'), subjects=config['subjects'], sessions=list(range(n_sessions)), minmax=True, n_classes='7+1', steady=True, image_like_shape=True).to('cuda')
        test_ds = DB6MultiSession(folder=os.path.expanduser('~/DB6'), subjects=config['subjects'], sessions=[n_sessions], minmax=(ds.X_min, ds.X_max), n_classes='7+1', steady=True, image_like_shape=True).to('cuda')

    from pickle import dump

    dump([ds.X_min, ds.X_max], open(f'ds_minmax_sessions={config["sessions"]}subjects={ ",".join(map(str, config["subjects"])) }.pickle', 'wb'))
    ...
    losses_accs = train(net, ds, k=0, bootstrap="no", test_ds=test_ds, training_config=config['training_config'])
    losses_accs_.append(losses_accs)
    print()

#net = ViT(**config)
#net.load_state_dict(torch.load("vit_epoch20.pth"))

from pickle import dump
from time import time

dump([configs, n_params_, losses_accs_], open(f'results_{time():.0f}.pickle', 'wb'))
