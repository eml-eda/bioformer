# -*- coding: utf-8 -*-
"""vit_db6_train_data_augmentation_2.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1yinEEvuTRlJI5V9LS5jDv7t6pkp0zdAm
"""
...
from src.datasets.utils import SuperSet
from src.datasets.db6 import DB6Session, DB6MultiSession
...
def windowing(X_instants, R_instants, Y_instants, v_Hz=2000, window_time_s=.150, relative_overlap=.7, steady=True, steady_margin_s=1.5):
    ...
    # formula: https://numpy.org/doc/stable/reference/arrays.indexing.html#basic-slicing-and-indexing
    # (NB: M_instants - r - r = M_instants - N because N = 2 * r)
    M = (M_instants - N) // slide + (((M_instants - N) % slide) > 0) #+ 1
...
def resample(X, R, y, factor):
    assert R.shape[0] == y.shape[0], "R and y must have the same shape"
    fR = interpolate.interp1d(np.arange(R.shape[0]), R, kind='nearest')
    fY = interpolate.interp1d(np.arange(y.shape[0]), y, kind='nearest')

    X_ = signal.resample(X, int(X.shape[0] * factor), axis=0)
    xp = np.linspace(0, R.shape[0] - 1, num=X_.shape[0])
    
    return X_, fR(xp), fY(xp)

def resample_and_windowing(X, R, y, factor, **kwargs):
    X, R, y = resample(X, R, y, factor=factor)
    return windowing(X, R, y, **kwargs)
...
class DB6SessionAugmented(Dataset):
    ...
    def windowing(self, steady=True, n_classes='7+1', image_like_shape=False, steady_margin_s=1.5, **kwargs):
        if str(n_classes) not in {'7+1', '7'}:
            raise ValueError('Wrong n_classes')

        self.image_like_shape = image_like_shape

        offsets, R_windows, Y_windows = windowing(self.X, self.R, self.Y, steady=steady, steady_margin_s=steady_margin_s, **kwargs)
        self.no_augmented_len = Y_windows.shape[0]

        self.time_stretches =  [.6, .7, .8, .9, 1.1, 1.2, 1.3, 1.4, 1.5]
        X0 = self.X
        # Time stretch d.a.
        for f in self.time_stretches:
            X_, (offsets_, R_windows_, Y_windows_) = resample_and_windowing(X0, self.R, self.Y, factor=f, steady_margin_s=steady_margin_s * f)
            offsets_ += self.X.shape[0]
            offsets = np.concatenate((offsets, offsets_), axis=0)
            R_windows = np.concatenate((R_windows, R_windows_), axis=0)
            Y_windows = np.concatenate((Y_windows, Y_windows_), axis=0)
            self.X = np.concatenate([self.X, X_])
        
        if n_classes == '7':
            # Filtra via finestre di non movimento
            mask = Y_windows != 0
            offsets, R_windows, Y_windows = offsets[mask], R_windows[mask], Y_windows[mask]
            # Rimappa label da 1-7 a 0-6
            Y_windows -= 1

        self.X = torch.tensor(self.X, dtype=torch.float32).permute(1, 0)
        self.Y = torch.tensor(Y_windows, dtype=torch.long)
        self.R = R_windows
        self.offsets = offsets

        # Important: the first must be 1 (the no_augmented part)
        self.amplitude_stretches = [1.]
        self.base_len = self.Y.shape[0]
    ...
    def __getitem__(self, idx):
        offset = self.offsets[idx % self.base_len] 
        x = self.X[:, offset:offset+300] * self.amplitude_stretches[idx // self.base_len]
        if self.image_like_shape:
            x = torch.unsqueeze(x, dim=1)
        return x, self.Y[idx % self.base_len]

    def __len__(self):
        return self.base_len * len(self.amplitude_stretches)

    def split(self, total_folds, val_fold=0):
        indices = np.arange(len(self))
        train_mask = np.tile(self.R, len(self.amplitude_stretches)) % total_folds != val_fold
        return Subset(self, indices[train_mask]), Subset(self, indices[~train_mask][:self.no_augmented_len])

class DB6MultiSessionAugmented(SuperSet):

    def __init__(self, subjects, sessions, folder='.', minmax=False, **kwargs):
        self.sessions = [DB6SessionAugmented(os.path.join(folder, f'S{subject}_D{(i // 2) + 1}_T{(i % 2) + 1}.mat')) for i in sessions for subject in subjects]
    ...
    for epoch in range(epochs):
        ...
        if 'each-epoch' in bootstrap or train_loader is None:
            ...
            if train_ds[0][0].device.type == 'cpu':
                ...
                train_loader_ = DataLoader(train_ds, batch_size=1024, shuffle=False, pin_memory=True, drop_last=False, num_workers=0)
                val_loader = DataLoader(val_ds, batch_size=1024, shuffle=False, pin_memory=True, drop_last=False, num_workers=0)
                if test_ds is not None:
                    test_loader = DataLoader(test_ds, batch_size=1024, shuffle=False, pin_memory=True, drop_last=False, num_workers=0)
            else :
...
def main_1test(chunk_idx):
    ...
    if ds_cache['ds_config'] != ds_config:
        ...
        ds = load(open("/home/fbmorghet/vit/exp9/session_augment_v2/train_ds.pickle", "rb"))
...
def main_train_subject(chunk_idx):
    ...
    ds = load(open(f"/home/fbmorghet/vit/exp8/session_augment_fix2/train_ds_subject{subject}_minmax10subjects.pickle", "rb"))
