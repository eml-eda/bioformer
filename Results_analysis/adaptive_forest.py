...
if __name__ == '__main__':
    ...
    pickle_name = 'ViT_1_1_8_8_1_60_0_0_14_0_0_results_finetune_0_5_1671187324.pickle'
    results_model2_05 = CPU_Unpickler(open(os.path.join(folder_results,pickle_name), 'rb')).load()
    pickle_name = 'ViT_1_1_8_8_1_60_0_0_14_0_0_results_finetune_5_10_1671188642.pickle'
    ...
    for patient in np.arange(10):
        ...
        for sess in np.arange(5):
            if sess == 0:
                model1_results = results_model1[1][patient%5]["val-fold"]["y_preds_steady"][sess].numpy()
                labels = results_model1[1][patient%5]["val-fold"]["y_trues_steady"][sess].numpy()
            else:
                model1_results = np.concatenate((model1_results,(results_model1[1][patient%5]["val-fold"]["y_preds_steady"][sess].numpy())),axis=0)
