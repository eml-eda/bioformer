# -*- coding: utf-8 -*-
"""vit_db6_test_init.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1r0kPSDbinB6KIeEqicSK_IFUJObVy_6k
"""
...
#! mkdir -p ~/DB6
#! cd ~/DB6 && wget -q --show-progress -r --no-parent --no-host-directories --cut-dirs=10 --accept 'S3_*' http://admin:nimda@lino1.francesco.pw/db6/
...
class ViT(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, pool = 'cls', channels = 3, dim_head = 64, dropout = 0., emb_dropout = 0., use_cls_token=True, sessions="ignore", optim_strategy='ignore'):
        super().__init__()
...
def train(net, ds, k, bootstrap, test_ds, epochs, optim_strategy):
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=50, gamma=0.5)
    #scheduler = torch.optim.lr_scheduler.CyclicLR(optimizer, base_lr=1e-7, max_lr=1e-3, step_size_up=100, step_size_down=None, mode='triangular', cycle_momentum=False) #exp_range o triangular
    
    #scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[3,5,7,9,12,15, 50], gamma=0.5)
    #scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=10)
    #epochs = 100
    """

    if optim_strategy == 'Adam:lr=1e-3,wd=0;StepLR:step=50,gamma=.5':
        optimizer = torch.optim.Adam(net.parameters(), lr=1e-3, betas=(.9, .999), weight_decay=0)
        scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=50, gamma=0.5)
    elif optim_strategy == 'Adam:wd=0;CyclicLR:base_lr=1e-7,max_lr=1e-3,step_size_up=100,step_size_down=None,mode=triangular':
        optimizer = torch.optim.Adam(net.parameters(), lr=0, betas=(.9, .999), weight_decay=0)
        ...
        raise ValueError("Wrong optim_strategy")


    torch.backends.cudnn.benchmark = True

    losses_accs = []
    y_trues_preds = {'trues': {}, 'preds': []}
    ...
    for epoch in range(epochs):
        ...
        if 'each-epoch' in bootstrap or train_loader is None:
            ...
            train_loader = DataLoader(train_ds, batch_size=64, shuffle=True, pin_memory=False, drop_last=True)
        ...
        if epoch == 0:
            train_loss, (y_pred, y_true), _ = get_loss_preds_scores(net, criterion, train_loader_)
            train_preds = y_pred.bincount(minlength=(y_true.max() + 1))
            train_acc = (y_true == y_pred).sum() / len(y_true)
            y_true_train = y_true#.tolist()

            val_loss, (y_pred, y_true), _ = get_loss_preds_scores(net, criterion, val_loader)
            val_preds = y_pred.bincount(minlength=(y_true.max() + 1))
            val_acc = (y_true == y_pred).sum() / len(y_true)
            y_true_val = y_true#.tolist()

            test_loss, (y_pred, y_true), _ = get_loss_preds_scores(net, criterion, test_loader)
            ...
            y_trues_preds['trues'].update({'y_true_train': y_true_train, 'y_true_val': y_true_val, 'y_true_test': y_true_test})
        ...
        train_loss, (y_pred, y_true), scores = get_loss_preds_scores(net, criterion, train_loader_)
        train_preds = y_pred.bincount(minlength=(y_true.max() + 1))
        train_acc = (y_true == y_pred).sum() / len(y_true)
        y_pred_train = y_pred#.tolist()
        scores_train = scores#.tolist()

        val_loss, (y_pred, y_true), scores = get_loss_preds_scores(net, criterion, val_loader)
        val_preds = y_pred.bincount(minlength=(y_true.max() + 1))
        val_acc = (y_true == y_pred).sum() / len(y_true)
        y_pred_val = y_pred#.tolist()
        scores_val = scores#.tolist()

        test_loss, (y_pred, y_true), scores = get_loss_preds_scores(net, criterion, test_loader)
        test_preds = y_pred.bincount(minlength=(y_true.max() + 1))
        test_acc = (y_true == y_pred).sum() / len(y_true)
        ...
        y_trues_preds['preds'].append({'y_pred_train': y_pred_train, 'y_pred_val': y_pred_val, 'y_pred_test': y_pred_test, 'scores_train': scores_train, 'scores_val': scores_val, 'scores_test': scores_test})

        #print(os.popen('free -h').read())
        #print(os.popen('uptime').read())

    return losses_accs, y_trues_preds

@torch.no_grad()
def get_loss_preds_scores(net, criterion, loader):
    y_pred, y_true = [], []
    scores = []
    loss = 0
    ...
    y_pred, y_true, scores = torch.cat(y_pred), torch.cat(y_true), torch.cat(scores)
    loss /= len(loader)
    
    return loss, (y_pred, y_true), scores
...
def train_subject(subject):
    ...
    configs_ = {
        ...
        "dim": 64, #[64, 14*20], #14*20, #14 * patch_size,
        ...
        "sessions": 5,
        "optim_strategy": ['Adam:lr=1e-3,wd=0;StepLR:step=50,gamma=.5', 'Adam:wd=0;CyclicLR:base_lr=1e-7,max_lr=1e-3,step_size_up=100,step_size_down=None,mode=triangular'], #'Adam:lr=1e-3,wd=0;StepLR:step=50,gamma=.5'
    }
    ...
    for i, config in enumerate(configs, start=1):
        ...
        for run_idx in range(10):
            ...
            if n_sessions != prev_n_sessions:
                ...
                ds = DB6MultiSession(folder=os.path.expanduser('~/DB6'), subject=subject, sessions=list(range(n_sessions)), minmax=True, n_classes='7+1', steady=True, image_like_shape=True).to('cuda')
                test_ds = DB6MultiSession(folder=os.path.expanduser('~/DB6'), subject=subject, sessions=[n_sessions], minmax=(ds.X_min, ds.X_max), n_classes='7+1', steady=True, image_like_shape=True).to('cuda')
            ...
            print(f"Config {i}/{len(configs)}, run {run_idx}/10")
            print(config)
            print(f"Params: {n_params / 10**6:.3f}M")
            losses_accs, y_trues_preds = train(net, ds, k=0, bootstrap="no", test_ds=test_ds, epochs=100, optim_strategy=config['optim_strategy'])
            #losses_accs_.append(losses_accs)
            #y_trues_preds_.append(y_trues_preds)
            print()

            runs.append({'losses_accs': losses_accs, 'y': y_trues_preds})
        
        runs_.append(runs)

    from pickle import dump

    dump([configs, n_params_, runs_], open(f'results_{subject}.pickle', 'wb'))
...
if __name__ == '__main__':
    ...
    with multiprocessing.Pool(PROCESSES) as pool:
        ...
        results = []
        for result in pool.imap_unordered(train_subject, subjects):
