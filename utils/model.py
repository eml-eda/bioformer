...
class Attention_quantized(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, dropout = 0.):
        super().__init__()
        inner_dim = dim_head *  heads
        project_out = not (heads == 1 and dim_head == dim)
        ...
        nn.init.uniform_(self.to_out[0].weight, -bound, bound)
        nn.init.uniform_(self.to_out[0].bias, -bound, bound)
        self.quant_out = torch.quantization.QuantStub()
...
class Transformer_quantized(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout = 0.):
        super().__init__()
        self.layers = nn.ModuleList([])
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(dim, Attention_quantized(dim, heads = heads, dim_head = dim_head, dropout = dropout)),
                PreNorm(dim, FeedForward(dim, mlp_dim, dropout = dropout))
            ]))
        ...
        self.dequant_4 = torch.quantization.DeQuantStub()

    def forward(self, x):
        for attn, ff in self.layers:
            x = self.dequant_2(attn(x)) + self.dequant_1(x)
            x = self.quant_1(x)
            x = self.dequant_3(ff(x)) + self.dequant_4(x)
            x = self.quant_2(x)
        return x
...
class ViT_quantized(nn.Module):
    def __init__(self, *, image_size, patch_size1, patch_size2, patch_size3, ch_1, ch_2, ch_3, tcn_layers, num_classes, dim_patch, depth, heads, pool = 'cls', channels = 3,
        ...
        nn.init.zeros_(self.cls_token)
        self.dropout = nn.Dropout(emb_dropout)
        self.transformer = Transformer_quantized(dim_patch, depth, heads, dim_head, dim_patch * 2, dropout)
        self.pool = pool
        self.to_latent = nn.Identity()
        self.mlp_head = nn.Sequential(
            nn.LayerNorm(dim_patch),
        ...
        self.dequant_final = torch.quantization.DeQuantStub()
    ...
    def forward(self, img):
        ...
        if self.use_cls_token:
            cls_tokens = repeat(self.cls_token, '() n d -> b n d', b = b)
            cls_tokens = self.quant1(cls_tokens)
            x = torch.cat((cls_tokens, x), dim=1)
            x = self.dequant1(x)
            x += self.pos_embedding[:, :(n + 1)]
        else :
