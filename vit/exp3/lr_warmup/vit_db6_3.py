...
for i, config in enumerate(configs, start=1):
    ...
    if n_sessions != prev_n_sessions:
        ...
        ds = DB6MultiSession(folder=os.path.expanduser('~/DB6'), subject=5, sessions=list(range(n_sessions)), minmax=True, n_classes='7+1', steady=True, image_like_shape=True)#.to('cuda')
        test_ds = DB6MultiSession(folder=os.path.expanduser('~/DB6'), subject=5, sessions=[n_sessions], minmax=(ds.X_min, ds.X_max), n_classes='7+1', steady=True, image_like_shape=True)#.to('cuda')
    ...
    losses_accs = train(net, ds, k=0, bootstrap="no", test_ds=test_ds, epochs=200)
    losses_accs_.append(losses_accs)
    print()

from pickle import dump

dump([configs, n_params_, losses_accs_], open(f'warmup.pickle', 'wb'))

net.transformer.layers[0][0].fn.to_qkv.weight.max()
#net.mlp_head[1].weight.max()

net.transformer.layers[0][0].fn.to_qkv.weight[0, :]

import matplotlib.pyplot as plt
# to_qkv[0] -> -0.3, 0.3 normale
plt.hist(net.transformer.layers[0][0].fn.to_qkv.weight[5, :].detach().cpu())

plt.hist(net.transformer.layers[0][0].fn.to_out[0].bias[:].detach().cpu())

plt.hist(net.transformer.layers[7][1].fn.net[3].weight[0,:].detach().cpu())

plt.hist(net.mlp_head[1].weight[0,:].detach().cpu())

plt.hist(net..bias[ :].detach().cpu())

# att
# xavier_normal gain=.75 per to_qkv
# xavier_normal gain=1 per to_out, nn.init.normal(std=.05)

# ff: xavier_normal gain=.75, nn.init.normal(std=.05)
a = torch.empty(64, 1)
nn.init.normal_(a, std=.2)
#nn.init.xavier_normal_(a, gain=.75)
plt.hist(a[:, 0])

import matplotlib.pyplot as plt
plt.hist(net.mlp_head[1].weight.detach().cpu())

ds, test_ds = get_ds()

ds.datasets[0].X.reshape(-1).mean(), ds.datasets[0].X.reshape(-1).std()
...
configs = [
    ...
    {'n_sessions': 1, 'bootstrap': 'no', 'use_cls_token': True, 'epochs': 100,},
    {'n_sessions': 5, 'bootstrap': '1_each-epoch', 'use_cls_token': False, 'epochs': 30,},
    {'n_sessions': 5, 'bootstrap': '1_each-epoch', 'use_cls_token': True, 'epochs': 30,},
    {'n_sessions': 5, 'bootstrap': 'no', 'use_cls_token': False, 'epochs': 100,},
    {'n_sessions': 5, 'bootstrap': 'no', 'use_cls_token': True, 'epochs': 100,},
]
...
for config in configs:
    ...
    config = {**config}
    n_sessions = config.pop('n_sessions')

    if n_sessions != prev_n_sessions:
        prev_n_sessions = n_sessions
        ds = DB6MultiSession(subject=5, sessions=list(range(prev_n_sessions)), minmax=True, n_classes='7+1', steady=True, image_like_shape=True)
        test_ds = DB6MultiSession(subject=5, sessions=[prev_n_sessions], minmax=(ds.X_min, ds.X_max), n_classes='7+1', steady=True, image_like_shape=True)
...
dump([configs, losses_accs_], open(f'train_bootstrap_vs_clstoken-5e-4-mean.pickle', 'wb'))